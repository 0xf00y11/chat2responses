...
- **Codex CLI integration & Auto-Heal** — Auto-configures Codex CLI to use the proxy, with non-destructive auto-healing at startup to safely align settings while preserving custom fields and creating backups
...
### Automatic Configuration Self-Check & Repair
...
- **Preserves Custom Fields**: This is a non-destructive repair—all other custom configuration blocks and fields inside your TOML file are left completely untouched.
...
- **设置向导** — 交互式命令行向导，快速配置
- **Codex CLI 集成与自动纠错** — 自动配置 Codex CLI 使用代理，并在每次启动服务时进行非破坏性的配置自检与自动修复（自动备份并保留其他自定义字段）
...
在启动代理服务时，`chat2responses` 会自动对 Codex CLI 的配置文件（`~/.codex/config.toml`）进行非破坏性校验与自动修复：
- **自动初始化**：如果配置文件不存在，将自动触发初始化配置。
- **配置对齐与自动纠错**：校验当前的 `model_provider`、`base_url` 和 `model` 是否指向当前运行的代理服务及选定模型。若存在不匹配，将创建备份（例如 `~/.codex/config.toml.chat2responses.bak`）并对齐关键配置。
- **保留自定义字段**：该过程为非破坏性修复，TOML 文件中的其他自定义配置块和字段将完整保留。
